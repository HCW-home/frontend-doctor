...
export class ConsultationService {
  ...
  init(currentUser) {
    ...
    this.socketEventsService.onConsultation().subscribe(c => {
      const existing = this.consultationsOverview.find(consultation => consultation._id === c.data._id);
      if(existing){
        return
      }
    ...
    this.socketEventsService.onConsultationAccepted().subscribe(event => {
      ...
      if (this.currentUser.id === event.data.consultation.acceptedBy) {
        ...
        this.consultationsOverview = this.consultationsOverview.filter(c => c._id !== event.data._id);
      }
  ...
  getConsultation(id): Observable<any> {
    ...
    return this.consultationsOverviewSub.pipe(map(cs => {
  ...
  acceptConsultation(id): Observable<any> {
    return this.http.post<any[]>(environment.api + `/consultation/${id}/accept`, null).pipe(tap(res => {
  ...
  closeConsultation(id): Observable<any> {
    return this.http.post<any[]>(environment.api + `/consultation/${id}/close`, null).pipe(tap(res => {
  ...
  readMessages(consultationId) {
    return this.http.post<any[]>(environment.api + `/consultation/${consultationId}/read-messages`, {}).subscribe(r => {
  ...
  updateUnreadCount() {
    ...
    this.unreadPendingSub.next(this.consultationsOverview.filter(c => c.consultation.status === 'pending').length);
  ...
  acceptNext() {
    const pending = this.consultationsOverview.filter(c => c.consultation.status === 'pending');
    if (pending[0]) {
  ...
  postFile(file: File, consultationId): Observable<any> {
    // debugger
    const endpoint = environment.api + `/consultation/${consultationId}/upload-file`;
  ...
  sendReport(file, consultationId): Observable<any> {
    const endpoint = environment.api + `/consultation/${consultationId}/send-report`;
    const formData: FormData = new FormData();
  ...
  saveConsultationFeedback(consultationId, rating, comment): Observable<any> {
    return this.http.post<any[]>(environment.api + `/consultation/${consultationId}/doctorFeedback`, { consultationId, rating, comment });
  }
