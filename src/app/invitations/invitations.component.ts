...
export class InvitationsComponent implements OnInit {
  ...
  openDialog(invite?, edit=false): void {
    if(invite.status === 'ACCEPTED' || invite?.translatorRequestInvite?.status === 'ACCEPTED') {
      this.dialog.open(InvitationAlreadyAcceptedComponent).afterClosed().subscribe(result => {
  ...
  getInvites(page) {
    ...
    this.page = page
    this.loading = true;
    this.invitations = this.inviteService.getInvitations((this.page - 1) * 10, 10).pipe(
      tap(resp => {
        console.log('response ', resp);
        this.totalCount = +(resp as HttpResponse<Invitation[]>).headers.get('X-Total-Count')
        console.log("total cout ", this.totalCount, )
        this.loading = false;
        this.currentInvites = (resp as HttpResponse<Invitation[]>).body;
        this.currentInvite = this.currentInvites.find(i => i.id === this.inviteId);
